...
class HierRoBERTaML(RobertaForTokenClassification):
    def __init__(self, config: ABSARoBERTaConfig, **kwargs):
        ...
        self.polarity_layer_2 = nn.Linear(config.num_aspect_labels + config.num_polarity_labels, config.num_polarity_labels)
        ...
        self.hier_loss = CTDLoss(device=config.device, aspect_func=self.aspect_detection, senti_func=self.polarity_detection)
        self.post_init()
    ...
    def forward(self,
        ...
        if a_labels is not None and p_labels is not None:
            loss, a_tags, p_tags = self.hier_loss(aspects=a_logits, polarities=p_logits, true_aspects=a_labels, true_polarities=p_labels,  mask=label_masks)
...
if __name__ == "__main__":
    ...
    print(model(input_ids, attention_mask=mask, alabels=alabels,
                slabels=slabels, valid_ids=valid_ids, label_masks=label_mask ))
