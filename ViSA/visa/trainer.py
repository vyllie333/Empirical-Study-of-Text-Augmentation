...
def validate(model, task, iterator, cur_epoch: int, is_test: bool = False):
    ...
    # Chạy từng bước trên bộ dữ liệu nhỏ
    with torch.no_grad():
    ...
    aspect_reports: dict = classification_report(eval_aspect_golds, eval_aspect_preds, output_dict=True, zero_division=0)
    senti_reports: dict = classification_report(eval_polarity_golds, eval_polarity_preds, output_dict=True, zero_division=0)
    ...
    aspect_label_map = ASPECT_LABELS 
    polarity_label_map = POLARITY_LABELS 
...
def train():
    ...
    config = ViSA_CONFIG_ARCHIVE_MAP[args.model_arch].from_pretrained(args.model_name_or_path, num_aspect_labels=len(aspect_label_map), num_polarity_labels=len(polarity_label_map))
    ...
    scheduler = get_cosine_schedule_with_warmup(optimizer, num_warmup_steps=train_steps_per_epoch, num_training_steps=args.epochs * train_steps_per_epoch)
    train_sampler = RandomSampler(train_set)
    train_iterator = DataLoader(train_set, sampler=train_sampler, batch_size=args.train_batch_size, num_workers=args.num_workers)
